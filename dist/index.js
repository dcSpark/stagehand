...
var functionString = getNodePath.toString();
  ...
  observe(_0) {
    return __async(this, arguments, function* ({
      ...
      for (const [index, xpaths] of Object.entries(evalResult.selectorMap)) {
        try {
          const xpath = xpaths[0];
          const { result } = yield this.stagehandPage.sendCDP("Runtime.evaluate", {
            expression: `document.evaluate('${xpath}', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue`,
            returnByValue: false
          });
          if (result.objectId) {
            const { node } = yield this.stagehandPage.sendCDP("DOM.describeNode", {
      ...
      yield this.stagehandPage.disableCDP("DOM");
      ...
      const elementsWithSelectors = yield Promise.all(
        observationResponse.elements.map((element) => __async(this, null, function* () {
          ...
          if (useAccessibilityTree) {
            const index = (_b = Object.entries(backendNodeIdMap).find(
              ([, value]) => value === elementId
            )) == null ? void 0 : _b[0];
            if (!index || !((_c = selectorMap[index]) == null ? void 0 : _c[0])) {
              const { object } = yield this.stagehandPage.sendCDP("DOM.resolveNode", {
...
var StagehandPage = class _StagehandPage {
  ...
  act(actionOrOptions) {
    return __async(this, null, function* () {
      ...
      const options = typeof actionOrOptions === "string" ? { action: actionOrOptions } : actionOrOptions;
      const {
        action,
        modelName,
        modelClientOptions,
...
function getBrowser(apiKey, projectId, env = "LOCAL", headless = false, logger, browserbaseSessionCreateParams, browserbaseSessionID, executablePath, downloadPath, browserArgs) {
  return __async(this, null, function* () {
    if (env === "BROWSERBASE") {
      if (!apiKey) {
        logger({
    ...
    if (env === "BROWSERBASE") {
      ...
      const downloadsPath = downloadPath || import_path.default.join(process.cwd(), "downloads");
      ...
      const mergedBrowserArgs = __spreadValues(__spreadValues({}, baseBrowserArgs), browserArgs);
      const browserArgsList = [];
      for (const [key, value] of Object.entries(mergedBrowserArgs)) {
        const transformedKey = key.replace(/([a-z])([A-Z0-9])/g, "$1-$2").toLowerCase();
        if (value === "" || value === true) {
